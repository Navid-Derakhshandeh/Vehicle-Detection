    ...
    public partial class Form1 : Form
    {
        ...
        private void Frame2(object sender, NewFrameEventArgs eventArgs)
        {
            ...
            foreach (Rectangle rectangle in rectangles)
            {
                using (Graphics graphics = Graphics.FromImage(bitmp2))
                {
                    using (Pen pen = new Pen(Color.Red, 2)) // Use a slightly thicker pen for visibility
                    {
                        graphics.DrawRectangle(pen, rectangle);
                    }
                }

                // Increment the car count for each detected rectangle
                carCount++;
            }
            ...
            // Update the count on the UI (assuming you have a label or textbox for this)
            Invoke((Action)(() =>
            {
                textBox1.Text = carCount.ToString(); // Update the count display
            }));
        }
